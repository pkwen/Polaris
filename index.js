  ...
  // app.get("/danny", (req, res) => {
    ...
    const wss = new SocketServer({ server: serverOnPort });
        ...
        const findAllRooms = (callback) => {
          db.collection('rooms').find({}).toArray((err, res) => {
            callback(res);
          })
        };
        ...
        setTimeout(()=>{console.log(codebase)}, 2000);


        const intervalSync = (callback) => {
          for(let j in codebase) {
            if(codebase[j].content) {
              db.collection('rooms').update({ roomID: codebase[j].roomID }, codebase[j], { upsert: true });
            }
          }
          callback(null, true);
        ...
        setInterval(() => {
          intervalSync(err => { console.log(err) });
        }, 60000)
